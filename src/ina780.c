...
static int16_t to_int16(uint8_t *data) { return (int16_t)((data[0] << 8) | data[1]); }
...
float ina780_read_power() {
  uint8_t buf[3];
  if (i2c_read(INA780_REG_POWER, buf, 3) != 3) return -1.0f;

  // 24-bit unsigned: POWER (480 µW/LSB)
  uint32_t raw = (buf[0] << 16) | (buf[1] << 8) | buf[2];
  return raw * 0.00048f;  // convert to Watts
}

double ina780_read_energy() {
  uint8_t buf[5];
  if (i2c_read(INA780_REG_ENERGY, buf, 5) != 5) return -1.0;

  // 40-bit unsigned: ENERGY (7.68 mJ/LSB)
  uint64_t raw = 0;
  for (int i = 0; i < 5; ++i) raw = (raw << 8) | buf[i];

  return raw * 0.00768 / 3600.0;  // convert to joules then Wh
}
